class role::fluidagent { 
  ...
  -> exec { 'Install agent':
    command  => '/usr/bin/sh -c "apt-get update && DEBIAN_FRONTEND=noninteractive apt-get install -y -o Dpkg::Options::=\"--force-confdef\" -o Dpkg::Options::=\"--force-confold\" fluid-agent"',
    user     => 'root',
    unless  => "/usr/bin/test -e /usr/bin/fluid-agent",
  }
}
